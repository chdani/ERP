...
import { CreateEditInventoryTransferComponent } from './create-edit-inventory-transfer/create-edit-inventory-transfer.component';
...
@Component({
  ...
  providers: [TranslatePipe, DialogService, DatePipe, CreateEditInventoryTransferComponent],
  animations: [
...
export class InventoryTransferComponent extends AppComponentBase implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    super(injector, 'SCR_INV_TRANSFER', 'allowView', _commonService)
    this.userComment = {
      comments: ""
    }
  }
  ...
  ngOnInit(): void {
    ...
    this._commonService.updatePageName(this._translate.transform("INV_TRANSFER"));
    this.getAll()
  ...
  export(exportType) {
    ...
    this.adSearch.exportHeaderText = this._translate.transform("INV_TRANSFER");
    var _url = "downloadInvtransfer";
    var fileDate = this._datePipe.transform(new Date(), "ddMMMyyyy_hhmm");
    let exportModel: ExportModel = {
      _fileName: this._translate.transform("INV_TRANSFER"),
  ...
  async searchRecords() {
    ...
    this.InventoryTransferList = [];

    this.expandedRows = {};
    let result = await this._webApiService.post('getInventoryTransferList', this.adSearch);
    if (result && result.length > 0) {
      this.disableExport = false;
      this.InventoryTransferList = result ?? of([]);
  ...
  async showDetail(event) {
    if (!event.data.details) {
      event.data.details = [];
      var result = await this._webApiService.get("getInventoryTransferDetList/" + event.data.id);
      if (result) {
  ...
  async processApproveOrReject(status: string) {
    ...
    if (this.selectedTrans) {
      ...
      var result = await this._webApiService.post("approveinventoryTransfer", this.selectedTrans);
      if (result) {
  ...
  createNewRecord() {
    this.router.navigate(["purchase/create-edit-inventory-transfer"], {
      state: { reqId: "" },
    });
  }

  async editRecord(selectedItem) {
    var result = await this._webApiService.get("getInventoryTransferDetList/" + selectedItem.id); // dummy request to check active session
    this.router.navigate(["purchase/create-edit-inventory-transfer"], {
  ...
  async deleteRecord(item) {
    this._confirmService.confirm({
      ...
      accept: async () => {
        item.active = "N";
        var result = await this._webApiService.post("saveInventoryTransfer", item)
        if (result) {
          var output = result as any;
          if (output.status == "DATASAVESUCSS") {
            this._toastrService.success(this._translate.transform("APP_SUCCESS"));
  ...
  getGridContextMenu(item) {
    ...
    if (item.status != "PURTRNSTSAPPROVED") {
      if (this.isGranted('SCR_INV_TRANSFER', this.actionType.allowEdit)) {
      ...
      if (this.isGranted('SCR_INV_TRANSFER', this.actionType.allowDelete)) {
      ...
      if (item.status != "PURTRNSTSREJECTED") {
        if (this.isGranted('SCR_INV_TRANSFER', this.actionType.allowApprove)) {
